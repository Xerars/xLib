...
# [變數]
CONFIG_WINDOW_TITLE = "GUI Simulate GPIO Monitor"
...
class GUI_GPIO_Window(QMainWindow):
  ...
  def GUI_Plot_Create(self):
    # 建立 Figure 物件 (figsize 可隨意設定，會自動隨視窗縮放)
    ...
    # 設定圖表標題與座標軸預設文字(空白狀態)
    self.Ax.set_title("GPIO Monitor - Waveform View",fontsize=12)
    ...
    Canvas = FigureCanvas(Fig)    # 將 Figure 包裝成 PyQt 可以使用的 Canvas
    return Canvas
  ...
  def GUI_Plot_Update(self,Data,Mode="Single"):
    # 1. 收集所有通道的清單
    All_Channels = []
    for Index,Info in sorted(Data["GPO"].items()):
      All_Channels.append(("GPO",Index,Info))
    for Index,Info in sorted(Data["GPI"].items()):
      All_Channels.append(("GPI",Index,Info))
    ...
    if TOT_Channel == 0:
      ...
      self.Ax.set_title("GPIO Monitor - No Data",fontsize=12)
    ...
    # 2. 根據模式與通道數動態決定佈局
    self.Canvas.figure.clear() # 清除整個 Figure
    ...
    # 將所有通道平均分配到決定好的圖數中，同時記錄全域的原始編號以對應顏色
    ...
    if Mode == "Single":
      ...
      for idx,ch in enumerate(All_Channels):
        ...
        Channel_Map[group_idx].append((idx,ch)) # 帶入全域 idx
    ...
    for i,Ax in enumerate(axes):
      if i < nPlots and i in Channel_Map and len(Channel_Map[i]) > 0:
        ...
        if Mode == "Single":
          Ax.set_title("GPIO Monitor - Waveform View (Single)",fontsize=12)
          Ax.set_xlabel("Time (s)",fontsize=10)
          Ax.set_ylabel("Level",fontsize=10)
        else:
          Ax.set_title(f"GPIO Group {i+1} (Channels: {len(Channel_Map[i])})",fontsize=10)
  ...
  def GUI_GetScreenSize(self):
    Screen   = QApplication.primaryScreen()     # 取得第一個螢幕物件
    Geometry = Screen.geometry()                # 取得可用的桌面大小
  ...
  def Menu_File_Load_Event(self):
    ...
    File_Path,File_Filter = QFileDialog.getOpenFileName(self,"選擇 LOG 檔案",                           # 對話框標題 
                                                           ".",                                        # 起始路徑
                                                           "Log Files (*.log *.txt);;All Files (*)")    # 檔案過濾器
    if File_Path:
      print(f"[Success]: Select File: {File_Path}")
      print(f"[Success]: Select File: {File_Filter}")
      Data = Parse_GPIO_Data(File_Path)

      if Data:
        self.Current_Data = Data    # 記住目前資料
        self.GUI_Plot_Update(Data,self.View_Mode)
        self.statusBar().showMessage(f"已載入檔案: {File_Path}",2000)     # 2s 後消失
      else:
        self.statusBar().showMessage(f"解析失敗: {File_Path}",2000)       # 2s 後消失
    else:
      print("[Info]: Cancel File Select")


  # Function: Menu_File_Save_Event
  # Descript: 處理圖表檔案儲存事件 (支援 PNG、JPG、PDF)
  def Menu_File_Save_Event(self):
    print("[Trig]: Menu File Save  Event")
    # 如果沒有載入資料或沒有圖表，跳出提示並返回
    if not self.Current_Data:
      QMessageBox.warning(self,"警告","目前沒有可儲存的圖表資料！請先載入 Log 檔案。")
      return

    # 開啟「儲存檔案」對話框，支援多種副檔名選項
    ...
    if File_Path:
      # 檢查使用者是否自行輸入了副檔名，若無則根據 Filter 自動補上
  ...
  def Menu_Info_Help_Event(self):
    ...
    QMessageBox.information(self,
      "說明",
      "【GUI Simulate GPIO Monitor 說明】\n\n"
        "1. 點擊[檔案]->[載入]讀取 GPIO Log\n"
        "2. 格式: 毫秒時間戳::[類型-編號]<名稱>:狀態\n"
        "   例如: 0::[GPO-0]<LED1>: 1\n"
        "3. 系統將自動解析 GPO 與 GPI 訊號並繪製波形。")
...
def Load_StyleSheet(App,FileName):
  try:
    ...
    print(f"Can Not Style Sheet Fie: {FileName}... Use Default")


# Function: Parse_GPIO_Data
# Descript: 解析 GPIO Log 檔案內容
def Parse_GPIO_Data(File_Path):
  Parse_Data = { "GPO":{},"GPI":{} }
  ...
  # 群組-4 名稱(字串)
  # 群組-5 準位(0/1)
  Pattern = re.compile(r"^\s*(\d+)::\[(GPO|GPI)-(\d+)\]<([^>]+)>:\s*([01])")
  ...
  try:
    ...
    print(f"[Success]: Parse Log Data Sucess!!")
    print(f"  - GPO Channel: {list(Parse_Data['GPO'].keys())}")
    print(f"  - GPI Channel: {list(Parse_Data['GPI'].keys())}")
    return Parse_Data
...
if __name__ == "__main__":
  ...
  Load_StyleSheet(App,CONFIG_STYLE_FILE)         # 載入外部 CSS 檔案
  Win = GUI_GPIO_Window()                       # 建立 Windows
  Win.show()                                    # 顯示 Windows
  sys.exit(App.exec())